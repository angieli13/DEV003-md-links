...
const extractLinks = (dirPath) => {
  ...
  const mdFilePaths = mdFiles(dirPath).map((file) => path.join(dirPath, file));

  // Leer el contenido de todos los archivos .md
  const mdPromises = mdFilePaths.map(filePath => readMd(filePath));
    ...
    .then(mdContents => {
      for (let i = 0; i < mdContents.length; i++) {
        ...
        while ((match = regEx.exec(mdContent)) !== null) {
          ...
          links.push({
            ...
            file: convertToAbsolutePath(mdFilePaths[i]), // Obtener la ruta absoluta del archivo
          });
        }
      }
      ...
      return Promise.all(subDirPromises)
        .then((subDirLinks) => {
          ...
          for (const linksInSubDir of subDirLinks) {
